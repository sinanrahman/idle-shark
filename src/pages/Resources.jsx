      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="flex space-x-4 mb-12">
          {['All', 'Blog', 'Case Studies'].map((filter, i) => (
            <button key={i} className={`px-6 py-2 rounded-full border ${i === 0 ? 'bg-white text-background border-white' : 'bg-transparent text-gray-400 border-gray-600 hover:border-white hover:text-white'} transition-colors`}>
              {filter}
            </button>
          ))}
        </div>
          ...
          {items.map((item, i) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 p-8 z-20">
                ...
                <h3 className="text-2xl font-heading font-medium text-white">{item.title}</h3>
              </div>
            </motion.div>
          ))}
        </div>
